...
import { canUser } from '/src/utilities/auth';
            ...
            {mainLink.children.map((childLink: any) => {
              ...
              return (
                <li key={childLink.url}>
                  <Link
                    to={childLink.url}
                    className="flex items-center p-1 text-base text-gray-900 transition duration-75 rounded-lg pl-11 group hover:bg-gray-100 dark:text-gray-200 dark:hover:bg-gray-700"
                  >
                    {t(childLink.title)}
                  </Link>
                </li>
